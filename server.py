...
from src.models.schemas import ZulipEvent
from src.config.input_parameters import parse_args
    ...
    for bot in available_bots:
        try:
            if bot.endswith(".py") and os.path.isfile(bot):
                ...
                module_name = "bots.bots.{bot}.{bot}".format(bot=bot)
...
app = FastAPI()
bots_config = {}  # type: Dict[str, Dict[str, str]]
init_data: Settings = Settings()


@app.post("/")
def handle_bot(event: ZulipEvent) -> Dict[Any, bool]:
    event = event.dict()
    ...
    assert init_data.bots_lib_modules[bot] is not None
    bot_handler = init_data.bot_handlers[bot]
    message_handler = init_data.message_handlers[bot]
    ...
    if is_mentioned:
        ...
        if message["content"] is None:
            return dict(response_not_required=True)
...
def initialize_data():
    ...
    init_data.bots_lib_modules = bots_lib_modules
    init_data.bot_handlers = bot_handlers
    init_data.message_handlers = message_handlers
    return init_data
